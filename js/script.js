...
const renderFilms = function(arr){
    ...
    for(let film of arr.Search){
        const html = `
    <a href="./info-movies/index.html" target="_blank">
        ...
        <button data-btn-id=${film.imdbID} class="films__btn-bookmark">Qidirish</button>
        </div>
    </a>
        `
...
function renderBookmark(bookmarks){
    ...
    for(let bookmark of bookmarks.Search){
        ...
        <p class="films__data">${bookmark.Year}</p>salom
        <button data-btn-id=${bookmark.imdbID} class="films__btn-bookmark">Salom</button>
        </div>
